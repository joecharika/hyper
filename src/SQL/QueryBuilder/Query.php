...
namespace Hyper\SQL\QueryBuilder {
    ...
    class Query
    {
         ...
         * @param bool $useCache
         */
        public function __construct(PDO $pdo = null, $table = null, $startQuery = '', $useCache = true)
        {
            try {
        ...
        {
            return $this->setQuery("default {$this->newParam($value)}");
        }

        private function newParam($value, $column = null)
        {
            $this->setParams($param = md5($column ?? uniqid()), $value);
        ...
        {
            $value = Str::contains($value, 'null') ?
                $value : $this->newParam($value, $column);
//            '"' . str_replace('\'', '\\\'', $value) . '"';
         ...
         */
        public function orderBy(array $columns, bool $asc = true)
        {
            $c = is_string($columns) ? $columns : '`' . Arr::spread($columns, false, '`,`') . '`';
            return $this->setQuery('order by ' . $c . ($asc ? '' : ' desc'));
        }
        ...
        {
            return "{$this->__toConfigString()} [{$this->getQuery()}]";
        }
        ...
        {
            $star = ucfirst(strtok($this->query, ' '));
            $params = count($this->params);
            return "{$star}Query(Table=$this->table,Cache=$this->useCache,Params=$params)";
        }
        ...
        public function exec($fetch = 'array', $fetchMode = 2): QueryResult
        {
            try {
                ...
                if ($this->useCache)
                    $queryResult = !empty(Database::$queries[strtr($this->query, $this->getParams())]) ?
                        (array_key_exists(strtr($this->query, $this->getParams()), Database::$queries)
                            ? $this->executedQuery(strtr($this->query, $this->getParams()))
                            : $this->executeQuery($this->query, $fetch, $fetchMode))
                        : $this->executeQuery($this->query, $fetch, $fetchMode);
                else $queryResult = $this->executeQuery($this->query, $fetch, $fetchMode);
                ...
                self::error(nl2br(
                        "<b>Query:</b>\n" . $this->getQuery() . "\t\n\n"
                        . "<b>ResolvedQuery:</b>\n" . strtr($this->getQuery(), $this->getParams()) . "\t\n\n"
                        . "<b>Params:</b>\n" . Arr::spread($this->getParams(), true, "\n") . "\t\n\n"
                        . "<b>Error: </b>\n" . $e->getMessage())
                );
            }
        ...
        private function executedQuery($query): QueryResult
        {
            Logger::log(
                $query,
                'DuplicateQuery(Skipped - Duplicate queries use cached result)',
                'last_query'
            );
        ...
        private function executeQuery($query, $fetch, $fetchMode): QueryResult
        {
            ...
            if (empty($this->params))
                Logger::log(strtr($query, $this->getParams()),
                    $this->__toConfigString(), 'last_query');

            else Logger::log("Query: " . $this->getQuery() . "\nResolvedQuery: " . strtr($query, $this->getParams()),
                $this->__toConfigString(), 'last_query');

            $q = (new QueryResult($query))
                ->setSuccess($success)
                ->setLastInsertedId($this->pdo->lastInsertId('id'))
                ->setAffectedRows($statement->rowCount());

            return isset($fetch) ?
                $q->setResult(Database::$queries[strtr($query, $this->getParams())] = $fetch === 'array'
