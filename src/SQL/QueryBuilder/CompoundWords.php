...
trait CompoundWords
{
     ...
     * @return Query
     */
    function createTable(string $tableName, array $columns, ?string $extras = ''): Query
    {
        $tableName = str_replace('`', '\'', $tableName);
        $this->create('table if not exists');
        $extras = strlen($extras ?? '') <= 0 ? '' : ",$extras";

        return $this->setQuery("`$tableName`(" . Arr::spread($columns) . "$extras)");
    }
