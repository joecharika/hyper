...
abstract class FileHandler
{
     ...
     * @return string
     */
    public static function uploadName(): string
    {
        return \uniqid(\strtr(\strtolower(HyperApp::instance()->name), [' ' => '-']) . '-');
    }

    public static function getReadableFileSize(int $bytes)
    {
        if ($bytes >= 1073741824) {
    ...
    public static function upload($file, $path = 'assets/uploads/')
    {
        ...
        if (move_uploaded_file($file->tmp_name, $targetFile)) {
            $newFileName = $targetDir . self::uploadName() . "." . $imageFileType;
