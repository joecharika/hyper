...
class DatabaseContext
{
    ...
    {
        $servername = HyperApp::config()->db->host;
        $username = HyperApp::config()->db->username;
        $database = HyperApp::config()->db->database;
        $password = HyperApp::config()->db->password;

        try {
            $conn = new PDO("mysql:host=$servername", $username, $password);

            !isset($conn) || (new HyperException())->throw("Failed to connect to server.");
            !$conn->query("CREATE DATABASE $database") || (new HyperException())->throw("Failed to create database.");
    ...
    public function sum($column, $limit = null, $offset = null): float
    {
        ...
        $stmt = $this->query("SELECT SUM(`$this->tableName`.`$column`) FROM `$this->tableName` WHERE `deletedAt` is NULL $isLimited $isOffset",
            [], true);
        if ($stmt->setFetchMode(PDO::FETCH_ASSOC)) {
    ...
    /**
     * TODO: Paginate result set
     * @param int $perPage
     * @param int $page
     * @return DatabaseContext
     */
    public function paginate($page = 1, $perPage = 20): DatabaseContext
    {
        if (empty($this->list))
            $this->select();

        $this->pagination = new Pagination($this->list, $page, $perPage);
